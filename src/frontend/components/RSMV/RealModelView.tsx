...
export const RealModelView: React.FC<RealModelViewProps> = ({ modelId, filePath, wireframe }) => {
    ...
    useEffect(() => {
        return () => {
            if (sceneNode && typeof sceneNode !== 'string') {
    ...
    useFrame((state) => {
        ...
        // Animate water shaders if they exist in the scene
        if (sceneNode && typeof sceneNode !== 'string') {
            ...
            sceneNode.traverse((obj) => {
                if ((obj as any).isMesh) {
                    ...
                    const updateMaterial = (m: THREE.Material) => {
                        ...
                        if (sm.uniforms) {
                            if (sm.uniforms.uWaterTickFade) {
                                sm.uniforms.uWaterTickFade.value[0] = time * 20.0; // Approximation of ticks
                                sm.uniforms.uWaterTickFade.value[1] = 0;
                            }
                            if (sm.uniforms.uTextureAnimationTime) {
                                sm.uniforms.uTextureAnimationTime.value = time * 1000.0;
                            }
                        }
    ...
    // The traverse logic has been moved into useFrame for dynamic updates
    // if requested (skip for schematic)
