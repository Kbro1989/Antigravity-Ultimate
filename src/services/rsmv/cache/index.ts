...
// 2. Helper Functions (No class dependencies)
...
// 4. Archive Helpers Dependent on CacheFileSource
...
export abstract class DirectCacheFileSource extends CacheFileSource {
	...
	async getFileArchive(meta: CacheIndex) {
		...
		if (this.getBuildNr() <= lastLegacyBuildnr) {
			const { parseLegacyArchive } = await import("./legacycache");
...
export function unpackSqliteBufferArchive(buffer: Buffer, subids: number[], namehashes: number[] | null): SubFile[] {
	...
	let offset = 0;
	let unknown = buffer.readUInt8(offset); offset++;
	let dataoffset = buffer.readUInt32BE(offset); offset += 4;
	let subbufs: SubFile[] = [];
	let scan = dataoffset;
	for (let i = 0; i < subids.length; i++) {
		let nextdataoffset = buffer.readUInt32BE(offset); offset += 4;
		let size = nextdataoffset - scan;
		subbufs.push({
			buffer: buffer.slice(scan, scan + size),
			offset: scan,
			size: size,
			fileid: subids[i],
			namehash: namehashes?.[i] ?? null
		});
		scan = nextdataoffset;
	}
	return subbufs;
