...
export type WorkerPackets = {
	...
	type: "putfile", major: number, minor: number, data: Uint8Array
} | {
	type: "getindex", major: number
} | {
	type: "save", major: number
} | {
	...
	try {
		switch (packet.type) {
			...
			case "getfile":
				...
				break;
			case "putfile":
				await putFile(packet.major, packet.minor, packet.data);
				(self as any).postMessage({ id, packet: true });
				break;
			case "getindex":
				...
				break;
			case "save":
				let dbData = await saveTable(packet.major);
				(self as any).postMessage({ id, packet: dbData });
				break;
		}
	} catch (err: any) {
...
async function putFile(major: number, minor: number, data: Uint8Array) {
	let { dbrun } = openTable(major);
	// We don't compress here yet, assuming data is already what we want to store or handled by caller
	// In Jagex cache, data is usually compressed.
	await dbrun(`INSERT INTO cache (KEY, DATA) VALUES (?, ?) ON CONFLICT(KEY) DO UPDATE SET DATA=excluded.DATA`, [minor, data]);
}

async function saveTable(major: number) {
	let { dbprom } = openTable(major);
	let db = await dbprom;
	return db.export(); // Returns Uint8Array of the DB file
}
