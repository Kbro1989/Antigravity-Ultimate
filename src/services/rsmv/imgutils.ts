...
export async function fileToImageData(file: Uint8Array, mimetype: "image/png" | "image/jpg", stripAlpha: boolean) {
	if (typeof ImageDecoder != "undefined") {
		...
		let img = new Image();
		let blob = new Blob([file.buffer as any], { type: mimetype });
		...
		let pixbuf = new Uint8ClampedArray(decoded.data.buffer as any, decoded.data.byteOffset, decoded.data.byteLength);
...
export async function pixelsToDataUrl(imgdata: ImageData) {
	if (typeof HTMLCanvasElement != "undefined") {
		...
		const req = (globalThis as any).__non_webpack_require__;
		const sharp = req("sharp") as typeof import("sharp");
		let pngfile = await sharp(imgdata.data as any, { raw: { width: imgdata.width, height: imgdata.height, channels: 4 } }).png().toBuffer();
...
export function sliceImage(img: ImageData, bounds: { x: number, y: number, width: number, height: number }) {
	...
	return new ImageData(newdata as any, bounds.width, bounds.height);
}
...
export function drawTexture(ctx: CanvasRenderingContext2D, img: ImageData | Texture | CanvasImage) {
	...
	if ("data" in img) {
		if (typeof ImageData != "undefined" && !(img instanceof ImageData)) {
			//@ts-ignore
			img = new ImageData(img.data as any, img.width, img.height);
		}
		cnv.width = img.width;
