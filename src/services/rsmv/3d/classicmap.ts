...
import { constModelsIds, EngineCache } from "./enginecache";
...
class ClassicMapBuilder {
    ...
    addDatfile(datfile: Buffer, level: number) {
        // ... (not used in current classicmap.ts? wait I see it in getClassicMapData)
    }
...
export async function classicOverlays(engine: EngineCache) {
    ...
    let usetga = !texindex;
    return Promise.all(config.tiles.map(async (q: any) => {
