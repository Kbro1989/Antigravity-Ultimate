...
import { SimpleModelDef, serializeAnimset, castModelInfo } from "./modelutils";
...
export async function avatarToModel(engine: EngineCache, buffer: Buffer, head: boolean) {
	...
	let playerkit = Object.fromEntries(playerkitarch.map((q: any) => [q.fileid, parse.identitykit.read(q.buffer, engine.rawsource)]));
