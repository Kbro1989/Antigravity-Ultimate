...
export function parseRT5Model(modelfile: Buffer, source: CacheFileSource) {
    ...
    let sortedFaceIndices = new Uint16Array(modeldata.facecount);
    for (let i = 0; i < modeldata.facecount; i++) sortedFaceIndices[i] = i;

    if (modeldata.facepriority) {
        sortedFaceIndices.sort((a, b) => (modeldata.facepriority![a] ?? 0) - (modeldata.facepriority![b] ?? 0));
    }

    for (let faceIdx = 0; faceIdx < modeldata.facecount; faceIdx++) {
        let i = sortedFaceIndices[faceIdx];
        srcindex0 = vertexindex[i * 3 + 0];
        srcindex1 = vertexindex[i * 3 + 1];
        srcindex2 = vertexindex[i * 3 + 2];
        ...
        if ((materialbuffer && materialbuffer[i] == 0 && (!colorbuffer || colorbuffer[i] == 0)) || (colorbuffer && colorbuffer[i] === 0 && !materialbuffer)) { continue; }
