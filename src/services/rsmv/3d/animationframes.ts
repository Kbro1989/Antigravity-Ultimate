...
export async function parseAnimationSequence4(loader: ThreejsSceneCache, sequenceframes: NonNullable<sequences["frames"]>): Promise<(model: ModelData) => AnimationClip> {
	...
	// let frames = Object.fromEntries(framearch.map((q: any) => [i + 1, parse.frames.read(q.buffer, loader.engine.rawsource)]));
	let frames = Object.fromEntries(framearch.map((q: any) => [q.fileid, parse.frames.read(q.buffer, loader.engine.rawsource)]));
